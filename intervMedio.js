...
function intervMedio(funcStr, a, b, tol = 1e-6, maxIter = 50) { //Si no se ingresa error -> se iguala a 1e-6 = 0.000001
  try {
    ...
    if (fa * fb > 0) {
      console.error("⚠ Error: f(a) y f(b) deben tener signos opuestos (Metodo de tanteos).");
      return null;
    }

    console.log("\nFunción ingresada:", funcStr);

    //Calculo de iteraciones previo al calculo
    let restaBmenosA = b - a;
    let logDeResta = log10(restaBmenosA);
    let logTol = log10(tol);
    let log2 = log10(2);
    let numIteraSinRedondeo = (logDeResta - logTol) / log2;
    let numIteraRedondeado = Math.ceil(numIteraSinRedondeo);

    console.log("Iteraciones previstas: ", numIteraRedondeado);
    ...
    for (let i = 1; i <= maxIter; i++) {
      ...
      //Condicion de convergencia -> resta de xi y xi-1 sea menor a tolerancia
      if (i !== 1) {
        let iteracionPrevia = iteraciones[i - 2];
        let iteracionActual = iteraciones[i - 1];
        let restaXi = iteracionActual.xi - iteracionPrevia.xi;
        if (Math.abs(restaXi) < tol) {
          console.log("\n✔ Convergencia alcanzada");
          return { raiz: xi, iteraciones };
        }
      }

      if (fa * fxi < 0) { //Cambio de signo y cambio en b
        b = xi;
        fb = fxi;
      } else { //Cambio de signo y cambio en a
        a = xi;
        fa = fxi;
      }
    }
